...
class WhatsAppUrduService {
  ...
  static const String receiptPromoFooter =
      'کاروبار کا سافٹ ویئر بنوانا ہے تو اس نمبر پر رابطہ کریں\n'
      '03331245518';
  ...
  /// Builds a full Zamindar profile summary (kisaans, debt, advance, ledger).
  static String buildZamindarProfileSummaryText({
    required String zamindarName,
    required String shopName,
    required double zamindarDebt,
    required double advancePayment,
    required int kisaanCount,
    required List<String> kisaanDebtLines,
    required List<String> ledgerLines,
    double? ledgerTotalValue,
    int? ledgerTotalQty,
    String? fatherName,
  }) {
    final name = zamindarName.trim().isEmpty ? 'محترم' : zamindarName.trim();
    ...
    final buffer = StringBuffer()
      ..writeln('السلام علیکم $name صاحب،')
      ..writeln()
      ..writeln('$shop کی طرف سے آپ کے پروفائل کا خلاصہ پیش خدمت ہے:')
      ..writeln();

    if (father.isNotEmpty) {
      buffer.writeln('والد/حوالہ: $father');
    }

    buffer
      ..writeln('کل کسان: $kisaanCount')
      ..writeln('زمیندار بقایا: ${formatAmount(zamindarDebt)}')
      ..writeln('ایڈوانس ادائیگی: ${formatAmount(advancePayment)}')
      ..writeln();

    buffer.writeln('کسان اور ان کا بقایا:');
    if (kisaanDebtLines.isEmpty) {
      buffer.writeln('• کوئی کسان ریکارڈ نہیں ملا');
    } else {
      for (final line in kisaanDebtLines) {
        final trimmed = line.trim();
        if (trimmed.isEmpty) continue;
        buffer.writeln('• $trimmed');
      }
    ...
    if (ledgerLines.isEmpty) {
      buffer.writeln('• کوئی لیجر اندراج نہیں ملا');
    } else {
      ...
      if (ledgerTotalQty != null || ledgerTotalValue != null) {
        buffer.writeln();
        if (ledgerTotalQty != null) {
          buffer.writeln('کل مقدار: $ledgerTotalQty');
        }
        if (ledgerTotalValue != null) {
          buffer.writeln('کل قیمت: ${formatAmount(ledgerTotalValue)}');
        }
      }
    }
  ...
  /// Opens WhatsApp with a full Zamindar profile summary.
  static Future<bool> sendZamindarProfileSummary({
    required String phone,
    required String zamindarName,
    required String shopName,
    required double zamindarDebt,
    ...
    final text = buildZamindarProfileSummaryText(
      zamindarName: zamindarName,
      shopName: shopName,
